...
# List of existing and new items
...
# This first agent is responsible for parsing the email sender info to populate a list of Item objects, as ListOfItems object.
...
# This second agent is responsible for searching the inventory database using the PineconeVectorSearchTool.
...
# This third agent is responsible for generating a quote for the OrderableItems object.
...
# This fourth agent is responsible for coming up with the email draft and calculating the total quote.
...
# This fifth agent is responsible for using the GMAIL_CREATE_EMAIL_DRAFT tool to create an email draft.
...
# This task is responsible for converting the information in the email message to a ListOfItems object.
...
# This task is responsible for searching the inventory database using the PineconeVectorSearchTool.
...
# This task is responsible for generating a quote for the OrderableItems object.
...
# This task is responsible for coming up with the email draft and calculating the total quote.
...
# This task is responsible for using the GMAIL_CREATE_EMAIL_DRAFT tool to create an email draft.
generate_email_draft_task = Task(
    ...
    You need to thoroughly check the information from the EmailDraft object to make sure you are providing the correct information to the customer.
    {sender_mail} is the email of the sender of the email.
    Check the {thread_id} to send the email to the correct thread.
    Use the {message} to draft an email that is consistent with what the sender of the email is saying.
    """,
